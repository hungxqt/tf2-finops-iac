...
- Các core reusable Terraform modules (networking, lakehouse, iam, compute-lambda, orchestration, alerting, observability, dashboard).
- Các môi trường composition đích (sandbox, staging, prod).
- Các hàm helper Go Lambda và validation unit tests.
...
- `scripts/package-lambdas.ps1` (Tạo mới/Thay đổi)
...
- `docs/SKELETON.md` (Tạo mới)
- `docs/SKELETON_vi.md` (Tạo mới)
- `environments/sandbox/` (Tạo các file cơ bản)
- `environments/staging/` (Tạo các file cơ bản)
- `environments/prod/` (Tạo các file cơ bản)
- `lambda_src/` (Tạo Go handlers, các package dùng chung, go.mod, và các file tests)
