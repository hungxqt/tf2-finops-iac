...
| `lambda_src/tests/fixtures/step_function_payloads.py` | 19 deterministic fixture dicts covering all workflow boundary scenarios |
| `lambda_src/tests/test_step_function_payload_contract.py` | 73 payload-contract tests in 9 groups (A-I) |

---

## Fixture Inventory (19 fixtures)
...
7. `POST_BUILD_DETECT_REQUEST` - BuildDetectRequest Pass state output ($.ai_detect_request)
8. `POST_INVOKE_DETECT_ANOMALY` - /v1/detect returns anomaly detected
9. `POST_INVOKE_DETECT_NO_ANOMALY` - /v1/detect returns clean (no anomaly)
10. `POST_INVOKE_DECIDE` - /v1/decide returns action_plan + rollback_payload
11. `POST_FORMAT_DECIDE_RESULT` - FormatDecideResult Pass state writes $.ai
12. `POST_ROUTER` - RouteAlert writes $.alert with finance+engineering routes
...
16. `POST_EXECUTE_CONTAINMENT` - ExecuteContainment result at $.containment
17. `POST_VERIFY_RESULT` - /v1/verify result at $.verify_result
18. `AI_FAIL_CLOSED_CONTEXT` - After SetAIFailClosedError, $.error populated
19. `CUR_DELAY_EXCEEDED_CONTEXT` - After SetCURDelayExceededError, cur_retry.count=4

---

## Test Groups (73 tests, 9 groups)
...
| D | Detect Path | 6 | /v1/detect shape, fail-closed on success=false and data_confidence=LOW |
...
```
73 passed in 0.14s (payload contract tests only)
84 passed in 0.39s (payload contract + state machine + lambda coverage + vpc alb caller)
187 passed in 5.74s (full suite, 0 failures)
```
...
2. **BuildDetectRequest** - `$.normalized.details.aws_cur_line_items` is readable and non-empty in healthy normalization
3. **InvokeDetect** - reads `path` from `$.ai_detect_request.path` (dynamic, confirmed `/v1/detect`)
4. **InvokeDecide body** - `anomaly_context.$` resolves to `$.ai_detect_response.anomalies_list[0]`
5. **CacheRollbackPayload** - DynamoDB putItem with `rollback_payload` serialized via `States.JsonToString`
6. **FormatDecideResult** - reads `action_plan[0].action` and `anomalies_list[0].anomaly_id`
7. **ReportVerifyResult** - `action_executed.target.$` resolves from `anomalies_list[0].resource_id`
8. **FailClosed/WriteCURDelayAudit** - all 9 required audit context fields resolve correctly
9. **EvaluateContainmentPolicy** - prod+destructive denial and dry-run denial both encoded in ASL
10. **SQS** - only `rollback_status_queue_url` queue in ASL; no detection queue
