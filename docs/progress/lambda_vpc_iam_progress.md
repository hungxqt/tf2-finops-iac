...
- **IAM Module Outputs (`modules/iam/outputs.tf`)**:
  - Added explicit `depends_on` inside the `lambda_role_arns` output block for worker VPC attachments (`aws_iam_role_policy_attachment.lambda_vpc`) and inline policies (`aws_iam_role_policy.state`, `cost_puller`, `normalizer`, `router`, `audit_writer`, `containment_worker`, `workers_xray`, and `workers_sqs`) to prevent the IAM propagation race.
- **SQS SendMessage Policy (`modules/iam/main.tf`)**:
  - Added inline policy (`aws_iam_role_policy.workers_sqs`) for all 7 Lambda worker roles to allow `sqs:SendMessage` on SQS queue ARNs (which includes the Lambda DLQ). This prevents AWS Lambda function creation failure (`InvalidParameterValueException: The provided execution role does not have permissions to call SendMessage on SQS`) when a dead-letter config is configured on the functions.
...
- [modules/compute-lambda/main.tf](file:///E:/code-folder/xbrain_projects/capstone_phase2_main/tf2-finops-iac/modules/compute-lambda/main.tf) (Modified)
...
## Results
- Terraform format and validate tests executed successfully across sandbox, staging, and prod environments.
- Trivy config scans on modified modules (modules/iam, modules/compute-lambda, modules/ai-runtime-lambda) reported 0 misconfigurations.
- Checkov static analysis scans passed successfully.
- Sandbox Terraform plan generated successfully: `Plan: 78 to add, 0 to change, 0 to destroy.` and saved to `lambda-vpc-iam.tfplan`.
