"""
tagger.py — Apply tags to a resource in the member account.

Tag action is permitted on ALL environments including prod.
Per deployment-contract.md §CDO Containment:
  tag-for-review: ec2:CreateTags, rds:AddTagsToResource — all environments
"""
...
from workers.containment_worker.model.output import TaggingResult
...
def execute_tag(
    ...
    """
    Apply FinOps review tags to resource.

    Tags applied:
      - FinOpsWatch = ReviewRequired
      - FinOpsAnomalyId = {anomaly_id}

    Dispatches to the correct AWS service based on resource_id.
    """
    tags_to_apply = {
    ...
    try:
        ...
        logger.error(
            "tagging failed",
            extra={"anomaly_id": inp.anomaly_id, "resource_id": resource_id, "error": str(exc)},
        )
        raise RuntimeError(f"TAGGING_FAILED resource={resource_id}: {exc}") from exc


def _tag_ec2(session: boto3.Session, resource_id: str, tags: dict[str, str]) -> TaggingResult:
...
def _tag_rds(session: boto3.Session, resource_id: str, tags: dict[str, str]) -> TaggingResult:
...
def _tag_sagemaker(session: boto3.Session, resource_id: str, tags: dict[str, str]) -> TaggingResult:
...
def compute_rollback_path_for_tag(tags_applied: dict[str, str]) -> dict[str, Any]:
    """Compute rollback_path for tag action — rollback = remove the applied tags."""
    return {
        "action": "remove_tags",
