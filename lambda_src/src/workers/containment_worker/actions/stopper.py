"""
stopper.py — Execute stop/shutdown action on non-prod resources.
...
Boundary check is already enforced in policy/boundary.py.
This file does NOT self-check boundaries — that is executor.py's responsibility.

Per deployment-contract.md §CDO Containment:
...
from workers.containment_worker.model.input import ContainmentInput, Boto3Payload
from workers.containment_worker.model.output import ApplyResult
...
def execute_apply(
    ...
    """
    Execute boto3 command from applied_payload.

    Uses applied_payload.boto3_equivalent from /v1/decide response —
    does not build its own command, only executes what the AI Engine recommended.
    """
...
def _dispatch_boto3(session: boto3.Session, payload: Boto3Payload) -> ApplyResult:
    """
    Dispatch boto3 call based on service + method from payload.

    Supported actions per deployment-contract.md:
    - ec2: stop_instances
    ...
    if service == "ec2" and method == "stop_instances":
        ...
        # Generic fallback — call any method if name matches
        boto3_method = getattr(client, method, None)
        if boto3_method is None:
            raise ValueError(f"Unsupported boto3 method: {service}.{method}")
