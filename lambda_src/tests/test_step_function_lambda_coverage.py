...
def test_step_function_lambda_coverage():
    ...
    expected_ps_workers = sorted(expected_source_workers + ["trigger"])
    assert ps_workers == expected_ps_workers, f"package-lambdas.ps1 workers list {ps_workers} does not match {expected_ps_workers}"
